...
@app.cell
def _(np, pd):
    filename = "../data/toxcast/toxcast_curated_CP"

    toxcast_annotations_df = pd.read_csv(
        f"{filename}.csv"
    ).set_index("Metadata_JCP2022")

    toxcast_annotations_df = (
        (
            toxcast_annotations_df.stack()
            .reset_index()
            .rename(
        ...
        .reset_index()
        .rename(columns={0: "Metadata_toxcast_list"})
    )

    toxcast_annotations_df
    return (toxcast_annotations_df,)
...
def _(pd):
    ...
    toxcast_df = pd.read_csv("../data/toxcast/toxcast_curated_CP.csv")

    print(f"Number of total compounds in this dataset: {len(compound_phenotypic_activity_df)}")

    print(f"Number of total compounds with a target in this dataset: {len(toxcast_df)}")
    ...
    toxcast_df = toxcast_df.set_index("Metadata_JCP2022")
...
@app.cell
def _(multi_label_col, pd, toxcast_annotations_df, utils):
    ...
    compound_df = compound_df.merge(
        toxcast_annotations_df, on="Metadata_JCP2022", how="inner"
    )

    ### Expand target list

    compound_df = (
        compound_df.assign(col = lambda x: x["Metadata_toxcast_list"].str.split("|"))
...
@app.cell
def _(compound_df, multi_label_col, toxcast_annotations_df):
    median_compound_df = compound_df.drop(['Metadata_Source',
           'Metadata_Plate', 'Metadata_Well', "Metadata_toxcast_list", "Metadata_target_list"], axis=1).groupby('Metadata_JCP2022').median().reset_index()
    median_compound_df
    ...
    ### Expand target list
    median_compound_df = (
        median_compound_df.assign(col = lambda x: x["Metadata_toxcast_list"].str.split("|"))
        .rename(columns={"col": f"{multi_label_col}"})
    )
    median_compound_df
